...
mongo_client = MongoClient("mongodb+srv://uberDB:uber_123@ubercluster.bwyio.mongodb.net/uber?retryWrites=true&w=majority")
...
@app.route("/registerBooking", methods=["POST"])
def add_booking():
    userName = request.json['userName']
    # userEmail = request.json['userEmail']
    sourceCity = request.json['sourceCity']
    destinationCity = request.json['destinationCity']
    dateTime = request.json['dateTime']
    booking = dict(userName=userName, sourceCity=sourceCity,
                date=datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                dateTime=dateTime, destinationCity=destinationCity, _id=str(ObjectId()))
